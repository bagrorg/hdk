...
bool need_to_hold_chunk(const Chunk_NS::Chunk* chunk,
  ...
  if (device_type == ExecutorDeviceType::CPU &&
    ...
    for (const auto target_expr : ra_exe_unit.target_exprs) {
      ...
      if (col_var && col_var->columnId() == chunk->getColumnId() &&
          col_var->tableId() == chunk->getTableId()) {
        return true;
      }
    }
  }
...
void ExecutionKernel::runImpl(Executor* executor,
  ...
  if (eo.executor_type == ExecutorType::Extern) {
    ...
    RowFuncBuilder row_func_builder(
        ra_exe_unit_, shared_context.getQueryInfos(), executor);
  ...
  if (ra_exe_unit_.groupby_exprs.empty()) {
    err = executor->executePlan(ra_exe_unit_,
                                ...
                                /*limit=*/-1,
                                start_rowid,
                                /*num_tables=*/ra_exe_unit_.input_descs.size(),
                                eo.allow_runtime_query_interrupt);
  } else {
...
void KernelSubtask::runImpl(Executor* executor) {
  ...
  if (kernel_.ra_exe_unit_.groupby_exprs.empty()) {
    err = executor->executePlan(kernel_.ra_exe_unit_,
                                compilation_result,
                                ...
                                /*limit=*/-1,
                                start_rowid_,
                                /*num_tables=*/kernel_.ra_exe_unit_.input_descs.size(),
