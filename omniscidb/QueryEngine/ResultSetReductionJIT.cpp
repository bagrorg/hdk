...
ReductionCode ResultSetReductionJIT::codegen() const {
  ...
  AUTOMATIC_IR_METADATA(cgen_state);
  auto ir_is_empty =
      create_llvm_function(reduction_code.ir_is_empty.get(), cgen_state, co);
  ...
  translate_function(
      reduction_code.ir_is_empty.get(), ir_is_empty, reduction_code, f, co);
  translate_function(reduction_code.ir_reduce_one_entry.get(),
                     ir_reduce_one_entry,
                     reduction_code,
                     f,
                     co);
...
#endif
  auto cpu_compilation_context = compiler::CPUBackend::generateNativeCPUCode(
